...
4. The final model is a soft Voting Ensemble of RandomForest (weighted 1:10 for false cutoffs), RBF SVC, and Logistic Regression.
5. Predict.py writes raw continuous probabilities `p_eot` to allow external calibration.
6. In honest 5-fold cross-validation, the model achieves an OOF AUC of 0.6720 and beats the starter model's honest English OOF delay (1135 ms vs. 1305 ms).
7. Hindi OOF Delay (826 ms) is lower than English because Hindi true/hold pauses are systematically shorter.
